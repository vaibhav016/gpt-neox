...
def _build_index_mappings(
    ...
    # if mpu.is_io_parallel():  # Only relevant for I/O parallel ranks
    torch.distributed.barrier(group=mpu.get_io_parallel_group())  # Explicit barrier
    counts = torch.cuda.LongTensor([1])
    torch.distributed.all_reduce(counts, group=mpu.get_io_parallel_group())
    torch.distributed.barrier(group=mpu.get_io_parallel_group()) # Barrier after all reduce as well.
    print_rank_0(f"Counts: {counts[0].item()}, World Size: {torch.distributed.get_world_size(group=mpu.get_io_parallel_group())}")
    assert counts[0].item() == torch.distributed.get_world_size(
    group=mpu.get_io_parallel_group()
)
    # torch.distributed.all_reduce(counts, group=mpu.get_io_parallel_group())
    # print(counts[0].item())
    # print(torch.distributed.get_world_size(group=mpu.get_io_parallel_group()))
    # assert counts[0].item() == torch.distributed.get_world_size(
    #     group=mpu.get_io_parallel_group()
    # )
...
def _load_index_mappings(
    ...
    if should_process_dataset:
        ...
        print("*********index_mapping_paths***************", index_mapping_paths["shuffle_idx_path"])
        print("******* idx_prefix**********", idx_prefix)
        ...
        num_samples_originally_seen = np.int64(np.int64(idx_prefix[-5][:-2]) / 1.005)
       
        ### ******* idx_prefix********** ['data/dclm/train', '400B/merged', 'train', 'train', '0', 'indexmap', '49072559ns', '2048sl', '1234s', 'packedpi', 'ac']
        seq_length_originally_seen = int(idx_prefix[-4][:-2])
