#!/bin/bash
#SBATCH --output=/ccs/home/vaibhav_016/bif151/scratch/vaibhav_016/slurm_logs/german/inf_cos/low/output/slurm-%j.out            
#SBATCH --error=/ccs/home/vaibhav_016/bif151/scratch/vaibhav_016/slurm_logs/german/inf_cos/low/error/slurm-%j.err
...
# Define variables
CHECKPOINT_DIR="checkpoints_3rd_dec/german/inf_cos/low"
...
# LATEST_CHECKPOINT="${CHECKPOINT_DIR}/JOB-2879938__it-47684_wu-0_mxlr-0.00015_mnlr-0.0_sch-constant_tr-german-train_resume"

# CHECKPOINT_PATH="${LATEST_CHECKPOINT}"


# Update load_low.yml with the checkpoint path
sed -i "s|\"load\":.*|\"load\": \"${CHECKPOINT_PATH}\",|" configs/load_checkpoints/german/3rd_dec/inf_cos/load_low.yml

# Run the training script
python deepy.py train.py --conf_dir configs llama2/410M.yml dataset_config/german/german_config_scratch.yml schedulers/const_inf_cos/annealing/adam_constant_lr1.5e-4_wu-0_low.yml load_checkpoints/german/3rd_dec/inf_cos/load_low.yml &                  
PYTHON_PID=$!
...
sed -i "s|\"finetune\":.*|\"finetune\": false,|" configs/load_checkpoints/german/3rd_dec/inf_cos/load_low.yml
